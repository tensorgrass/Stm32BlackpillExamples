...
#define SAMPLES  64
uint16_t buffer[SAMPLES]; // Guarda el estado completo de GPIOB cada 1 ms
/* USER CODE END 0 */
...
int main(void)
{
  ...
  // Iniciar el DMA: periférico=GPIOB->IDR, memoria=buffer, longitud=SAMPLES
  HAL_DMA_Start_IT(&hdma_tim2_ch3_up, (uint32_t)&GPIOB->IDR, (uint32_t)buffer, SAMPLES);

  // Habilitar request de DMA por actualización de TIM2
...
static void MX_DMA_Init(void)
{
  ...
  __HAL_LINKDMA(&htim2, hdma[TIM_DMA_ID_UPDATE], hdma_tim2_ch3_up);
...
void HAL_DMA_ConvCpltCallback(DMA_HandleTypeDef *hdma)
{
    ...
    {
        // Procesa el buffer aquí: buffer[0..SAMPLES-1]
        // El bit 13 de cada muestra indica el estado de PB13 en ese instante.
        // Ejemplo:
        for (int i = 0; i < SAMPLES; ++i) {
            uint8_t pb13 = (buffer[i] & (1 << 13)) ? 1 : 0;
            // ... tu lógica de procesamiento ...
        }

        // Si quieres muestreo continuo, reinicia el DMA:
        HAL_DMA_Start_IT(hdma, (uint32_t)&GPIOB->IDR, (uint32_t)buffer, SAMPLES);
    }
}
/* USER CODE END 4 */
