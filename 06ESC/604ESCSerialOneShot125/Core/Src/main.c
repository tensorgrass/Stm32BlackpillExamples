...
#define COUNTER_PERIOD_START 9000 // COUNTER_PERIOD_END / 2
#define COUNTER_PERIOD_END 18000 //
...
//  HAL_Delay(1000);
  esc_value = map_esc_values(motor_value, 0, 4095, COUNTER_PERIOD_START, COUNTER_PERIOD_END); // Map The ADC Reading To PWM DutyCycle
  TIM3->CCR3 = esc_value;
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
        ...
        esc_value = map_esc_values(motor_value, 0, 4095, COUNTER_PERIOD_START, COUNTER_PERIOD_END); // Map The ADC Reading To PWM DutyCycle
        int num_char = snprintf(output_value, sizeof(output_value) ,"Valor %lu %lu\r\n", motor_value, esc_value);
